...
public class MaxSpinner extends AbstractSpinnerModel {
    int value = 0;
    int minimum;
    int maximum;
    int stepSize;
    MinSpinner minSpinner = null;

    public MaxSpinner(int value, int minimum, int maximum, int stepSize) {
        this.value = value;
    ...
    public void setMinSpinner(MinSpinner spinner) {
        minSpinner = spinner;
    }
    ...
    public void setValue(Object value) {
        if (value instanceof Integer) {
            ...
            if (minSpinner != null) {
                if (minSpinner.getIntValue() > this.value) {
                    minSpinner.setValue(this.value);
                }
            }
            fireStateChanged();
        }
